...
# LOAD THE PACKAGES
library(dlnm) ; library(splines) ; library(MASS); library(dplyr)
...
forecast_cond<-c("b2mp","b2mr","b2mq")
...
for (j in 1:length(forecast)){
  ...
  #for (k in 1:length(regions)){
    ...
    obs_sub <- obs %>% filter(!is.na(death)) %>% # removing days with missing mortality data
    ...
    obs_ext<-reformat_ncf_to_DLNM(ncf_dat=paste0(climate_dir,forecast[j], "/250Ensemble_EricCoord_", reg1,"_", 
                                                    forecast_cond[j],"_2021-06-18.nc", sep=""),
                                      var = 't2m', is_ERA5=F)
    ...
    #boundary change in Vancouver only
    if (reg1=="vancouver"){
